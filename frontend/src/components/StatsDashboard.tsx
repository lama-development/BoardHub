...
export function StatsDashboard() {
  ...
  const [playerStats, setPlayerStats] = React.useState<PlayerStatistics | null>(
    null,
  );
  ...
  const loadOverview = React.useCallback(async () => {
    ...
    try {
      ...
      setSelectedSessionId(
        (current) => current ?? nextSessions.at(0)?.sessionId ?? null,
      );
      setTournaments(nextTournaments);
      setSelectedTournamentId(
        (current) => current || nextTournaments.at(0)?.tournamentId || "",
      );
    } catch (loadError) {
      setError(
  ...
  return (
    <main className="min-h-screen px-4 py-5 text-[#111111] sm:px-6 sm:py-7">
          ...
          <strong className="text-xl leading-none">
            {
              new Set(
                sessions.flatMap((session) =>
                  session.participants.map((player) => player.playerReference),
                ),
              ).size
            }
          </strong>
      ...
      <nav
        className="mx-auto mt-4 flex w-full max-w-7xl flex-wrap gap-2"
        aria-label="Sezioni statistiche"
      >
        {(
          [
      ...
      <section className="mx-auto mt-4 w-full max-w-7xl">
        {error ? (
          <AlertBanner tone="danger" icon={<AlertCircle size={18} />}>
            {error}
          </AlertBanner>
        ) : null}
        {view === "sessions" ? (
        ...
        {view === "player" ? (
          <PlayerLookup
            isSearching={isSearchingPlayer}
            playerReference={playerReference}
            playerStats={playerStats}
            onChange={setPlayerReference}
...
function SessionHistory({
  ...
  if (isLoading)
    return <EmptyState>Caricamento dello storico sessioni…</EmptyState>;
  if (!sessions.length)
    return <EmptyState>Nessuna sessione conclusa disponibile.</EmptyState>;
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.35fr)]">
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <StatusChip tone="info">
                    {session.participants.length} gioc.
                  </StatusChip>
                </div>
              </button>
            </li>
      ...
      {selectedSession ? (
        ...
        <EmptyState>Seleziona una sessione per vedere il risultato.</EmptyState>
      )}
    </div>
  );
}
        ...
        <table className="w-full min-w-160 text-left text-sm">
          <thead className="bg-[#f7f1e4] text-xs uppercase">
            <tr>
              ...
              <th className="px-3 py-3 text-right">Punti</th>
              <th className="px-3 py-3 text-right">Mov.</th>
              <th className="px-3 py-3 text-right">Trappole</th>
            </tr>
          </thead>
          <tbody>
            {session.participants.map((player) => (
              ...
              >
                ...
                <td className="px-3 py-3">
                  <StatusChip tone={player.survived ? "success" : "danger"}>
                    {player.survived ? "Sopravvissuto" : "Abbattuto"}
                  </StatusChip>
                </td>
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.2fr)]">
      <Surface className="h-fit p-4">
        ...
        <form className="mt-4 flex gap-2" onSubmit={onSubmit}>
          <label className="sr-only" htmlFor="player-reference">
      ...
      {playerStats ? (
        ...
        <EmptyState>
          Le statistiche aggregate del giocatore compariranno qui.
        </EmptyState>
      )}
    </div>
  );
...
function PlayerStatsCard({ stats }: { stats: PlayerStatistics }) {
  const metrics = [
    ["Punti totali", stats.totalPoints, Trophy, "yellow"],
    ["Sessioni", stats.sessionsPlayed, Swords, "purple"],
    [
      "Sopravvivenza",
    ...
    ["Celle percorse", stats.totalCellsTravelled, Footprints, "cyan"],
    ["Tiri riusciti", stats.totalSavesSucceeded, ShieldCheck, "pink"],
    ["Danni subiti", stats.totalDamageTaken, AlertCircle, "pink"],
  ] as const;
  return (
    <Surface className="overflow-hidden">
...
function TournamentLeaderboard({
  ...
  if (isLoading && !selectedTournament)
    return <EmptyState>Caricamento dei tornei…</EmptyState>;
  ...
  return (
    <Surface className="overflow-hidden">
      <div className="flex flex-col gap-3 border-b-2 border-[#111111] bg-[#fff7d1] p-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl">
            {selectedTournament?.name ?? "Seleziona un torneo"}
          </h2>
        </div>
        <label className="text-sm font-bold" htmlFor="tournament-select">
          Torneo
          <select
            className="bh-input mt-1 block h-10 min-w-64 px-3 font-normal"
          ...
          >
            {tournaments.map((tournament) => (
              ...
              >
                {tournament.name} · {tournament.gameType}
              </option>
            ))}
          </select>
        </label>
      ...
      {isLoading ? (
        <div className="p-6 text-center text-sm font-semibold">
          Aggiornamento classifica…
        </div>
      ) : !leaderboard.length ? (
        <EmptyState>
          Questo torneo non ha ancora risultati in classifica.
        </EmptyState>
      ) : (
          ...
          <table className="w-full min-w-170 text-left text-sm">
            <thead className="bg-[#f7f1e4] text-xs uppercase">
              <tr>
                ...
                <th className="px-3 py-3 text-right">Tiri</th>
                <th className="px-3 py-3 text-right">Celle</th>
              </tr>
            </thead>
            <tbody>
